...
type SimpleDeploymentLister interface {
	...
	List(selector labels.Selector) (ret []*v1.SimpleDeployment, err error)
...
// List lists all SimpleDeployments in the indexer.
func (s *simpleDeploymentLister) List(selector labels.Selector) (ret []*v1.SimpleDeployment, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.SimpleDeployment))
	})
	return ret, err
}
...
type SimpleDeploymentNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.SimpleDeployment, error)
...
// List lists all SimpleDeployments in the indexer for a given namespace.
func (s simpleDeploymentNamespaceLister) List(selector labels.Selector) (ret []*v1.SimpleDeployment, err error) {
...
// Get retrieves the SimpleDeployment from the indexer for a given namespace and name.
func (s simpleDeploymentNamespaceLister) Get(name string) (*v1.SimpleDeployment, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("simpledeployment"), name)
	}
	return obj.(*v1.SimpleDeployment), nil
}
